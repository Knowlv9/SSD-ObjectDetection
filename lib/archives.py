...
dirname = "./input/train_20220815/LC20_1024"
...
class SSDDateset(Dataset):
	...
	def __getitem__(self, idx):
		...
		if not self.is_test:
			annotation_path = self.annotation_folder_path + file.replace(".png", '')
			# annotation_path = self.annotation_folder_path + file.split('.')[0]
			# print(annotation_path, os.path.exists(annotation_path))
			# sys.exit()
	...
	def get_xy(self, annotation):
		...
		annotation = bs4(annotation, "xml")

		xmin = int(annotation.find("xmin").string)
		xmax = int(annotation.find("xmax").string)
		ymin = int(annotation.find("ymin").string)
		ymax = int(annotation.find("ymax").string)
